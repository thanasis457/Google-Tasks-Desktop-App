...
def selectionchange(window,all_tasks,lists,reload,submit,service,i):
...
def delete_button(window,all_tasks,list_id,task_id,service):
    replyreply = QMessageBox.question(window,'Quit', 'Are you sure you want to delete?',\
...
def main():
    ...
    edit_box=QLineEdit()
    all_tasks.addWidget(edit_box,1,0)
    submit=QPushButton('Submit')
    all_tasks.addWidget(submit,1,1)
    submit.clicked.connect(partial(submit_button,edit_box,lists[0][1]))
    # print(lists)
    for i in lists:
    ...
    for cnt,i in enumerate(tasks):
        ...
        label.setFont(QFont('Arial',14))
        label.adjustSize()
        all_tasks.addWidget(label,cnt+2,0)
        x=QPushButton("Delete")
        y=QPushButton("Complete")
        all_tasks.addWidget(x,cnt+2,1)
        all_tasks.addWidget(y,cnt+2,2)
    ...
    # print(all_tasks.count())
    drop_list.currentIndexChanged.connect(partial(selectionchange,window,all_tasks,lists,reload,submit,service))
    # all_tasks.change
